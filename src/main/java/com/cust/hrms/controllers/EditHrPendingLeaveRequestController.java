...
@WebServlet("/editHrPendingLeaveRequest")
public class EditHrPendingLeaveRequestController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		RequestDispatcher rd = request.getRequestDispatcher("hrSupervisorLeavePendingApproval.jsp");
		...
		if(commentLength < 1 && (ls.getCode().equals("declined") || ls.getCode().equals("sent_back_for_correction"))) {
			...
			// Update leave request details in the database table
			...
			if(count >= 1) {
				...
				int employeesId[] = erd.getRoleEmployeesId(hrRoleId);
				String hrEmailAddress[] = ed.getEmployeeEmailsArray(employeesId);
				...
				String resumptionDate = dd.changeFormatDate(l.getResumptionDate());
				if(ls.getCode().equals("declined")) {
					...
					if(he.isEmailEnable()) {
						...
						if(l.getSecondaryReliefOfficeId() == 0) {
							String reliefOfficerEmailAddress[] = { ed.getEmployeeEmail(l.getPrimaryReliefOfficeId())};
							lem.getReliefOfficerLeaveRequestApprovedMessage(reliefOfficerEmailAddress, reliefOfficerData);
						}
						else {
							String reliefOfficerEmailAddress[] = { 
				...
				session.setAttribute("pageTo", "hrLeavePendingApprovalWithPay.jsp");
